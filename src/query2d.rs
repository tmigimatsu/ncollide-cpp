...
#[repr(C)]
pub struct ncollide2d_query_ray_intersection_t {
    toi: f64,
    normal: [f64; 2],
    // feature: usize,
...
pub extern fn ncollide2d_query_project_point(shape: Option<&nc::shape::ShapeHandle<f64>>,
                                             ...
                                             out_projection: Option<&mut ncollide2d_query_point_projection_t>) {
...
pub extern fn ncollide2d_query_closest_points(
        ...
        max_dist: f64, out_p1: Option<&mut [f64; 2]>, out_p2: Option<&mut [f64; 2]>)
...
pub extern fn ncollide2d_query_contact(
        m1: Option<&ncollide2d_math_isometry_t>, g1: Option<&nc::shape::ShapeHandle<f64>>,
        m2: Option<&ncollide2d_math_isometry_t>, g2: Option<&nc::shape::ShapeHandle<f64>>,
        prediction: f64, out_contact: Option<&mut ncollide2d_query_contact_t>) -> bool {
    let m1 = isometry_from_raw(m1.unwrap());
    let m2 = isometry_from_raw(m2.unwrap());
    let g1 = g1.unwrap().as_ref();
    let g2 = g2.unwrap().as_ref();
...
#[no_mangle]
pub extern fn ncollide2d_query_ray_new(ptr_origin: Option<&[f64; 2]>, ptr_dir: Option<&[f64; 2]>)
        -> *mut nc::query::Ray<f64> {
    let origin = na::Point2::from_slice(ptr_origin.unwrap());
    let dir = na::Vector2::from_column_slice_generic(na::U2, na::U1, ptr_dir.unwrap());
    let ray = nc::query::Ray::new(origin, dir);
    Box::into_raw(Box::new(ray))
}

#[no_mangle]
pub unsafe extern fn ncollide2d_query_ray_delete(ptr: *mut nc::query::Ray<f64>) {
    Box::from_raw(ptr);
}
...
pub extern fn ncollide2d_query_toi_and_normal_with_ray(shape: Option<&nc::shape::ShapeHandle<f64>>,
                                                       ...
                                                       solid: bool,
                                                       out_intersect: Option<&mut ncollide2d_query_ray_intersection_t>) -> bool {
    use nc::query::RayCast;
